...
function gui_init() {
    ...
    $(document).on('click', '#sa-gen-setup', save_device_object_and_setup_sa_gen); // entering SaGen Setup WebPage
...
function _save_device_object(do_id) {
    if (!do_id) {
        do_id = 0;
    }
    let data = {
...
function save_device_object_and_setup_sa_gen(){
    let do_id = this.getAttribute('do_id');
    _save_device_object(do_id);
...
function show_save_gif (after_object, id='save-gif') {
    $('#'+id).remove()
    var img = $('<img>', {
        'src': '/iottalk/ccm/static/images/save.gif',
        'id': id,
    });
