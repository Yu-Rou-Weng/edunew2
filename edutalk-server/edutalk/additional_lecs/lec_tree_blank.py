...
# update sky when receiving Luminance data
def on_Luminance_I(data):
    if data != None:
        Change_background(data)

# tree data = [[len, pos, direction], ...]
tree_sample = []
...
# compute tree data in [[len, pos, direction], ...]
tree(startingbranch, startingposition, startingdirection)
# sort by len
def compareByLen(e):
    pass # replace this line with your anwser
...
for i in range(len(tree_sample)):
    tree_object.append(cylinder(
    	#pos=fill your anwser,
        #axis=fill your anwser,
...
# set initial tree
create_tree((Humidity_I//10)+1)

# update tree when receiving humidity data
...
def update_info():
    global label_info
    label_info.text = 'Humidity_I: {:.2f}\nLuminance_I: {:.2f}'.format(Humidity_I, Luminance_I)
